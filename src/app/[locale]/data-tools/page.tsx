...
import { generateVatReturn } from '@/lib/flowvia-functions';
...
export default function DataToolsPage() {
  ...
  const [vatPeriod, setVatPeriod] = useState<string>(format(new Date(), 'yyyy-MM'));
  ...
  const handleGenerateVatReturn = async () => {
    if (!companyId) return;
    try {
        const res: any = await generateVatReturn({ companyId, period: vatPeriod });
        toast({ title: 'VAT Return Generated', description: `Net VAT for ${vatPeriod}: ${res?.netVatMinor ?? 0}` });
    } catch (e: any) {
        console.error(e);
        toast({ variant: 'destructive', title: 'Failed to generate VAT return', description: e.message });
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4">
                <div className="space-y-2">
                    <Label>VAT Return (Monthly)</Label>
                    <Input type="month" value={vatPeriod} onChange={(e) => setVatPeriod(e.target.value)} />
                </div>
                <div className="flex items-end">
                    <Button onClick={handleGenerateVatReturn} disabled={!vatPeriod}>
                        Generate VAT Return
                    </Button>
                </div>
            </div>
