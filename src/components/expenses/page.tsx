...
import { addDoc, serverTimestamp, Timestamp, doc, updateDoc, deleteDoc, type FieldValue } from 'firebase/firestore';
...
const safeGetDate = (date: DateInput): Date | null => {
  ...
  // FieldValue sentinel (serverTimestamp)
  return null;
};
...
export default function ExpensesPage() {
  ...
  const handleAddExpense = async (expenseData: Omit<DailyExpense, 'id' | 'createdAt' | 'createdBy' | 'companyId'>) => {
    ...
    try {
        ...
        await addDoc(expensesCollectionRef, withCompanyId(companyId, {
            ...expenseData,
                ...
                <TableBody>
                  {filteredExpenses.map((expense) => (
                    <TableRow key={expense.id}>
                      ...
                      <TableCell className="text-right font-medium">{formatMoneyMinor(toMinor(expense.amount, expense.currency), expense.currency)}</TableCell>
