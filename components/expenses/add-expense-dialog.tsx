...
interface AddExpenseDialogProps {
  ...
  onAddExpense: (expense: Omit<DailyExpense, 'id' | 'createdAt' | 'createdBy' | 'companyId'>) => void;
  sellers: Seller[];
  employees: Employee[];
}

export default function AddExpenseDialog({ open, onOpenChange, onAddExpense, sellers, employees }: AddExpenseDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    if (expenseType === 'salary' && !salaryRecipientType) {
        toast({ variant: 'destructive', title: t('recipientTypeRequired'), description: t('pleaseSelectARecipientType') });
        return;
    }
    ...
    const expenseData: Omit<DailyExpense, 'id' | 'createdAt' | 'createdBy' | 'companyId'> = {
      expenseType,
      description,
    ...
    if (expenseType === 'salary') {
        if(salaryRecipientType === 'seller') {
            expenseData.paid_to_seller_id = sellerId;
        } else if (salaryRecipientType === 'employee') {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{t('addANewExpenseToTheLog')}</DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-3 rounded-md border p-4">
                ...
                <RadioGroup value={salaryRecipientType} onValueChange={(v) => setSalaryRecipientType(v as 'seller' | 'employee')} className="flex gap-4">
                    <div className="flex items-center space-x-2">
                        <RadioGroupItem value="seller" id="r_seller" />
                        <Label htmlFor="r_seller">{t('seller')}</Label>
                    </div>
                     <div className="flex items-center space-x-2">
                        <RadioGroupItem value="employee" id="r_employee" />
                        <Label htmlFor="r_employee">{t('employee')}</Label>
                    </div>
                </RadioGroup>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={handleSubmit}>{t('addExpense')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
