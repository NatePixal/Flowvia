...
export default function AddExpenseDialog({ open, onOpenChange, onAddExpense, sellers, employees }: AddExpenseDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    const expenseData: any = {
      ...
      currency,
      date,
      paid_to_seller_id: expenseType === 'salary' && salaryRecipientType === 'seller' ? sellerId : '',
      employee_id: expenseType === 'salary' && salaryRecipientType === 'employee' ? employeeId : '',
      ...
      {ready && (
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{t('expenses.addANewExpenseToTheLog')}</DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-3 rounded-md border p-4">
                  ...
                  <RadioGroup value={salaryRecipientType} onValueChange={(v) => setSalaryRecipientType(v as 'seller' | 'employee')} className="flex gap-4">
                      <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="r_seller">{t('employees.seller')}</Label>
                      </div>
                       <div className="flex items-center space-x-2">
                          <RadioGroupItem value="employee" id="r_employee" />
                          <Label htmlFor="r_employee">{t('employees.employee')}</Label>
                      </div>
                  </RadioGroup>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleSubmit}>{t('expenses.addExpense')}</Button>
          </DialogFooter>
        </DialogContent>
      )}
    </Dialog>
