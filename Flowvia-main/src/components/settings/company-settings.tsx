        ...
        <FancyCard>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4 rounded-md border p-4">
                    <Label className="font-semibold">{t('settings.shopFeatures')}</Label>

                    <div className="flex items-center justify-between gap-4">
                      <div className="space-y-1">
                        <p className="text-sm font-medium">{t('settings.enableLocations')}</p>
                        <p className="text-xs text-muted-foreground">{t('settings.enableLocationsHelp')}</p>
                      </div>
                      <Switch checked={locationsEnabled} onCheckedChange={setLocationsEnabled} disabled={!canEdit || isSaving} />
                    </div>

                    <div className="flex items-center justify-between gap-4">
                      <div className="space-y-1">
                        <p className="text-sm font-medium">{t('settings.enablePOS')}</p>
                        <p className="text-xs text-muted-foreground">{t('settings.enablePOSHelp')}</p>
                      </div>
                      <Switch checked={posEnabled} onCheckedChange={setPosEnabled} disabled={!canEdit || isSaving} />
                    </div>

                    <div className="flex items-center justify-between gap-4">
                      <div className="space-y-1">
                        <p className="text-sm font-medium">{t('settings.telegramAlerts')}</p>
                        <p className="text-xs text-muted-foreground">{t('settings.telegramAlertsHelp')}</p>
                      </div>
                      <Switch checked={telegramAlertsEnabled} onCheckedChange={setTelegramAlertsEnabled} disabled={!canEdit || isSaving} />
                    </div>

                    {telegramAlertsEnabled && (
                      <div className="space-y-2">
                        <Label htmlFor="telegramChatId">{t('settings.telegramChatId')}</Label>
                        <Input id="telegramChatId" value={telegramChatId} onChange={(e) => setTelegramChatId(e.target.value)} disabled={!canEdit || isSaving} placeholder={t('settings.telegramChatIdPlaceholder')} />
                        <p className="text-xs text-muted-foreground">{t('settings.telegramTokenHint')}</p>
                      </div>
                    )}
                </div>
