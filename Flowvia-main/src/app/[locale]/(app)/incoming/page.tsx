...
import { recordIncomingFifoFn } from '@/lib/flowvia-functions';
...
export default function IncomingPage() {
  ...
  const { firestore, companyId, userProfile, user, companyBaseCurrency, locationsEnabled } = useFirebase();
  ...
  const handleAddIncomingProduct = async (incoming: {
    ...
    if (!locationsEnabled) {
      toast({ variant: 'destructive', title: t('toast.error.title'), description: t('incoming.enableLocationsFirst') });
      return;
    }

    try {
      await recordIncomingFifoFn({
        companyId,
        ...
        fx: incoming.fx,
        incomeDate: incoming.incomeDate.toISOString(),
      });
      toast({ title: t('toast.success.productAdded'), description: t('incoming.incomingRecorded') });
