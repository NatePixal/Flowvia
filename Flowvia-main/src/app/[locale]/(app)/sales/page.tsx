...
import { issueInvoiceForSale, generateInvoicePrintable, recordSaleFifoFn, refundSaleFifoFn } from '@/lib/flowvia-functions';
...
export default function SalesPage() {
  ...
  const { firestore, companyId, user, userProfile, companyBaseCurrency, locationsEnabled } = useFirebase();
  ...
  const canRefund = hasPermission(userProfile, 'sales', 'refund');
  ...
  const handleAddSale = async (saleData: any) => {
    ...
    if (locationsEnabled && !saleData?.locationId) {
      toast({ variant: 'destructive', title: t('toast.error.missingFields'), description: t('sales.locationRequired') });
      return;
    }

    try {
      const fx = saleData?.fx;
      const res: any = await recordSaleFifoFn({
        companyId,
        locationId: saleData.locationId,
        clientId: saleData.clientId,
        sellerId: saleData.sellerId,
        sellerName: sellers.find(s => s.id === saleData.sellerId)?.name || '',
        paymentType: normalizePaymentType(saleData.paymentType),
        currency: saleData.salePriceCurrency,
        fx,
        date: saleData.date?.toISOString?.() || saleData.date,
        discount: 0,
        vatRate: 0,
        paidAtSale: extractPaidAtSaleMajor(saleData),
        items: [{
          productId: saleData.productId,
          quantity: saleData.quantity,
          unitPrice: saleData.salePrice,
        }],
      } as any);

      await recomputeClientOutstanding(firestore, companyId, saleData.clientId);

      toast({ title: t('toast.success.saleRecorded'), description: t('toast.success.saleRecordedSuccessMessage') });
      setIsAddDialogOpen(false);
      return res;
    } catch (e: any) {
      console.error('[handleAddSale] Failed:', e);
  ...
  const handleRefundSale = async (saleId: string, clientId: string) => {
    if (!companyId || !firestore || !canRefund) return;
    const reason = window.prompt(t('sales.refundReasonPrompt')) || '';
    try {
      await refundSaleFifoFn({ companyId, saleId, reason });
      await recomputeClientOutstanding(firestore, companyId, clientId);
      toast({ title: t('sales.refunded'), description: t('sales.refundedDesc') });
    } catch (e: any) {
      toast({ variant: 'destructive', title: t('toast.error.title'), description: e?.message || t('toast.error.unexpectedError') });
    }
  };
                      ...
                      <TableCell className="text-center">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {canRefund && locationsEnabled && (
                              <DropdownMenuItem onClick={() => handleRefundSale(sale.id, sale.clientId)} className="text-destructive">
                            ...
                            {locationsEnabled ? (
                              <DropdownMenuItem disabled>
                                <span>{t('sales.fifoEditDisabled')}</span>
                              </DropdownMenuItem>
                            ) : (
                              <>
